...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
    ...
    <xsl:template match="tei:TEI">
        ...
        \usepackage{xecolor}
        \usepackage{hyperref}
        \hypersetup{pdftex,colorlinks=true,linkcolor=black,urlcolor=black,pdftitle={Anaphora of Mark the Evangelist}}
        \usepackage{hypcap}
        \usepackage{morefloats}
        \let\myfont\normalfont
        \usepackage{fancyhdr}
        \pagestyle{fancy}
        \chead{\myfont\textit{Anaphora of Mark the Evangelist}}
    ...
    <xsl:template match="tei:sourceDesc">
        <xsl:text>\pdfbookmark[0]{Manuscript}{manuscript}</xsl:text>
