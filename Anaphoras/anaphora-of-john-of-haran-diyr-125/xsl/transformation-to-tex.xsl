...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
    ...
    <xsl:template match="tei:sourceDesc">
        ...
        <!--<xsl:text>\item Country: </xsl:text>
    ...
    <xsl:template match="tei:seg[@type = 'heading']">
        <xsl:text>\setLR </xsl:text>
        <xsl:text>\pdfbookmark[1]{Heading}{heading}</xsl:text>
        <xsl:value-of select="@n"/>
        <xsl:text>\par\vspace{3mm}</xsl:text>
        <xsl:text>\setRL </xsl:text>
        <xsl:text>\LR{\marginpar{\textbf{</xsl:text>
        <xsl:value-of select="child::tei:pb/@n"/>
        <xsl:text>}}}\nolinebreak </xsl:text>
        <xsl:text> $|$ \nolinebreak </xsl:text>
        <xsl:apply-templates select="child::tei:lb | text() | child::tei:hi"/>
    </xsl:template>
